...
import { Search, Plus, Pencil, Ticket, X, Trash2, AlertCircle, Loader2 } from 'lucide-react';
...
const EMPTY_FORM = { codigo: '', tipo: 'PORCENTAJE', valor: 0, fecha_expiracion: '', usos_maximos: 0 };
type FormState = typeof EMPTY_FORM;

function expiry(c: Coupon) { return c.fecha_expiracion || c.fecha_fin; }
function maxUses(c: Coupon) { return c.usos_maximos ?? c.usos_maximo; }
...
export default function CouponsAdminPage() {
  ...
  async function toggleActive(coupon: Coupon) {
    try {
      const updated = await couponsApi.toggleStatus(coupon.id);
      setCoupons((prev) => prev.map((c) => c.id === coupon.id ? updated : c));
      toast.info(updated.activo ? 'Cupón activado' : 'Cupón desactivado');
    } catch (err: any) {
      const msg = err?.response?.data?.message;
      toast.error(Array.isArray(msg) ? msg.join(', ') : msg || 'No se pudo cambiar el estado');
    }
  }
  ...
  function openEdit(coupon: Coupon) {
    ...
    setEditForm({
      ...
      tipo: coupon.tipo,
      valor: coupon.valor,
      fecha_expiracion: expiry(coupon) || '',
      usos_maximos: maxUses(coupon) || 0,
    });
  }
  ...
  async function handleCreateSubmit(e: React.FormEvent) {
    ...
    try {
      const newCoupon = await couponsApi.create({
        ...
        fecha_expiracion: createForm.fecha_expiracion,
        ...(createForm.usos_maximos > 0 && { usos_maximos: Number(createForm.usos_maximos) }),
      });
      ...
      const msg = err?.response?.data?.message;
      toast.error(Array.isArray(msg) ? msg.join(', ') : msg || 'No se pudo crear el cupón');
    } finally {
      setCreating(false);
    }
  }
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    ...
    try {
      const updated = await couponsApi.update(editingCoupon.id, {
        ...
        usos_maximos: Number(editForm.usos_maximos),
      });
      ...
      const msg = err?.response?.data?.message;
      toast.error(Array.isArray(msg) ? msg.join(', ') : msg || 'No se pudo actualizar el cupón');
    } finally {
      setEditing(false);
    }
  }
  ...
  async function handleDelete(id: number) {
    ...
    try {
      ...
      const msg = err?.response?.data?.message;
      toast.error(Array.isArray(msg) ? msg.join(', ') : msg || 'No se pudo eliminar el cupón');
    } finally {
      setDeleting(false);
    }
  }

  const filtered = coupons.filter((c) => c.codigo.toLowerCase().includes(search.toLowerCase()));
              ...
              {loading ? (
                ...
                <tr key={coupon.id} className="hover:bg-zinc-900/50 transition-colors">
                  <td className="px-4 py-3">
                    <span className="font-mono font-bold text-red-500 bg-red-500/10 px-2 py-1 rounded-md text-xs">{coupon.codigo}</span>
                  </td>
                  <td className="px-4 py-3 text-zinc-400 text-xs">{coupon.tipo === 'PORCENTAJE' ? 'Porcentaje' : 'Valor Fijo'}</td>
                  <td className="px-4 py-3 font-medium text-zinc-100">
                    {coupon.tipo === 'PORCENTAJE' ? `${coupon.valor}%` : `L${coupon.valor}`}
                  </td>
                  <td className="px-4 py-3 text-zinc-400 text-xs">{formatDate(expiry(coupon))}</td>
                  <td className="px-4 py-3 text-zinc-400 text-xs">{coupon.usos_actuales ?? 0} / {maxUses(coupon) || '∞'}</td>
                  <td className="px-4 py-3">
                    <button onClick={() => toggleActive(coupon)} className="flex items-center gap-2 group">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(coupon)} className="p-1.5 rounded-lg text-zinc-400 hover:bg-zinc-800 hover:text-blue-400 transition-colors" title="Editar">
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button onClick={() => setDeletingId(coupon.id)} className="p-1.5 rounded-lg text-zinc-400 hover:bg-zinc-800 hover:text-red-400 transition-colors" title="Eliminar">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
              ...
              <button onClick={() => setShowCreate(false)} className="text-zinc-500 hover:text-white transition-colors p-1"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleCreateSubmit} className="p-6 space-y-4">
              <Field label="Código del Cupón" required>
                <input type="text" required placeholder="EJ: CINE2026" className={`${inputCls} font-mono uppercase`}
                  value={createForm.codigo} onChange={(e) => setCreateForm({ ...createForm, codigo: e.target.value })} />
              </Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Tipo" required>
                  <select className={inputCls} value={createForm.tipo} onChange={(e) => setCreateForm({ ...createForm, tipo: e.target.value })}>
                    <option value="PORCENTAJE">Porcentaje (%)</option>
                    <option value="FIJO">Valor Fijo (L)</option>
                  </select>
                </Field>
                <Field label="Valor" required>
                  <input type="number" required min={1} className={inputCls}
                    value={createForm.valor} onChange={(e) => setCreateForm({ ...createForm, valor: Number(e.target.value) })} />
                </Field>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Vencimiento" required>
                  <input type="date" required className={inputCls}
                    value={createForm.fecha_expiracion} onChange={(e) => setCreateForm({ ...createForm, fecha_expiracion: e.target.value })} />
                </Field>
                <Field label="Límite de Usos">
                  <input type="number" min={0} placeholder="0 = ilimitado" className={inputCls}
                    value={createForm.usos_maximos} onChange={(e) => setCreateForm({ ...createForm, usos_maximos: Number(e.target.value) })} />
                </Field>
              </div>
              <div className="flex justify-end gap-3 pt-4 border-t border-zinc-800 mt-6">
                <button type="button" onClick={() => setShowCreate(false)} className="px-4 py-2 rounded-xl text-xs font-bold text-zinc-400 hover:bg-zinc-800 transition-colors">CANCELAR</button>
                <button type="submit" disabled={creating} className="px-6 py-2 rounded-xl text-xs font-bold text-white bg-red-600 hover:bg-red-700 transition-all shadow-lg shadow-red-900/20 disabled:opacity-50 flex items-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
              ...
              <button onClick={() => setEditingCoupon(null)} className="text-zinc-500 hover:text-white transition-colors p-1"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
              <Field label="Código del Cupón" required>
                <input type="text" required className={`${inputCls} font-mono uppercase`}
                  value={editForm.codigo} onChange={(e) => setEditForm({ ...editForm, codigo: e.target.value })} />
              </Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Tipo" required>
                  <select className={inputCls} value={editForm.tipo} onChange={(e) => setEditForm({ ...editForm, tipo: e.target.value })}>
                    <option value="PORCENTAJE">Porcentaje (%)</option>
                    <option value="FIJO">Valor Fijo (L)</option>
                  </select>
                </Field>
                <Field label="Valor" required>
                  <input type="number" required min={1} className={inputCls}
                    value={editForm.valor} onChange={(e) => setEditForm({ ...editForm, valor: Number(e.target.value) })} />
                </Field>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Vencimiento" required>
                  <input type="date" required className={inputCls}
                    value={editForm.fecha_expiracion} onChange={(e) => setEditForm({ ...editForm, fecha_expiracion: e.target.value })} />
                </Field>
                <Field label="Límite de Usos">
                  <input type="number" min={0} className={inputCls}
                    value={editForm.usos_maximos} onChange={(e) => setEditForm({ ...editForm, usos_maximos: Number(e.target.value) })} />
                </Field>
              </div>
              <div className="flex justify-end gap-3 pt-4 border-t border-zinc-800 mt-6">
                <button type="button" onClick={() => setEditingCoupon(null)} className="px-4 py-2 rounded-xl text-xs font-bold text-zinc-400 hover:bg-zinc-800 transition-colors">CANCELAR</button>
                <button type="submit" disabled={editing} className="px-6 py-2 rounded-xl text-xs font-bold text-white bg-red-600 hover:bg-red-700 transition-all shadow-lg shadow-red-900/20 disabled:opacity-50 flex items-center gap-2">
