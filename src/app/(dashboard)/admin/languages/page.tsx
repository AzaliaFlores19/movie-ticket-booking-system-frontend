...
import { Search, Plus, Pencil, Trash2, X, AlertCircle, Languages } from 'lucide-react';
...
export default function LanguagesAdminPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    try {
      const newItem = await languagesService.create({ nombre: formName });
      setItems([newItem, ...items]);
      setShowCreate(false);
      setFormName('');
      toast.success('Idioma creado correctamente');
    } catch (error) {
      const mock = { id: Date.now(), nombre: formName };
      setItems([mock, ...items]);
      setShowCreate(false);
      setFormName('');
      toast.success('Idioma creado (Modo Local)');
    }
  }
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    try {
      const updated = await languagesService.update(editingItem.id, { nombre: formName });
      setItems(items.map(i => i.id === editingItem.id ? updated : i));
      setEditingItem(null);
      setFormName('');
      toast.success('Idioma actualizado correctamente');
    } catch (error) {
      setItems(items.map(i => i.id === editingItem.id ? { ...i, nombre: formName } : i));
      setEditingItem(null);
      setFormName('');
      toast.success('Idioma actualizado (Modo Local)');
    }
  }
  ...
  async function handleDelete(id: number) {
    try {
      ...
      setItems(items.filter(i => i.id !== id));
      setDeletingId(null);
      toast.success('Idioma eliminado (Modo Local)');
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Idiomas</h1>
          <p className="text-sm text-zinc-400 mt-1">Administrar idiomas de las películas</p>
        </div>
        <button
      ...
      <div className="bg-zinc-950 border border-zinc-800/60 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 border-b border-zinc-800/60 flex items-center justify-between flex-wrap gap-4">
          ...
          <span className="text-xs text-zinc-500">{filtered.length} idioma{filtered.length !== 1 ? 's' : ''}</span>
        </div>
        ...
        <table className="w-full text-sm text-left">
          <thead>
            <tr className="border-b border-zinc-800/60 bg-zinc-900/30">
              ...
              <th className="px-6 py-4 text-xs font-bold text-zinc-400 uppercase tracking-widest">Nombre del Idioma</th>
          ...
          <tbody className="divide-y divide-zinc-800/40">
            {loading ? (
              <tr>
                <td colSpan={3} className="px-6 py-12 text-center text-zinc-500">Cargando idiomas...</td>
              </tr>
            ) : paginated.length === 0 ? (
              <tr>
                <td colSpan={3}>
                  <div className="flex flex-col items-center justify-center py-16 gap-3">
                    <Languages className="w-10 h-10 text-zinc-700" />
                    <p className="text-sm text-zinc-500 font-medium">No se encontraron idiomas</p>
                  </div>
                </td>
              </tr>
            ) : (
              paginated.map((item, index) => (
                <tr key={item.id} className="hover:bg-zinc-900/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className="font-semibold text-zinc-100 group-hover:text-red-400 transition-colors">{item.nombre}</span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => { setEditingItem(item); setFormName(item.nombre); }}
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
              <h2 className="text-lg font-bold text-white tracking-tight">
                {showCreate ? 'Agregar Idioma' : 'Editar Idioma'}
              </h2>
            ...
            <form onSubmit={showCreate ? handleCreate : handleEdit} className="p-6 space-y-4">
              <Field label="Nombre del Idioma" required>
                <input
                  type="text"
                  required
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">¿Eliminar idioma?</h3>
